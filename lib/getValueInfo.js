...
module.exports = (source, config) => {
  ...
  return _.pairs(obj.props).reduce((rst, pair) => {
    ...
    rst.push({
      ...
      defaultValue: stringify(info.defaultValue.value, config.type === 'markdown' ? '`' : '<code>', config.type === 'markdown' ? '`' : '</code>')
    });
    ...
    if (info.type.name === 'shape') {
      ...
      _(type.value).pairs().value().map(tp => {
        return {
          ...
          type: humanize(tp[1]),
          defaultValue: stringify(defaultValue[tp[0]], config.type === 'markdown' ? '`' : '<code>', config.type === 'markdown' ? '`' : '</code>')
        };
      }).forEach(sub => {
        rst.push(sub);
      });
